...
export class BaseService<E extends BaseModel> extends EggBaseService {
  static serviceMap: Map<string, BaseService<any>> = new Map();
  ...
  static getService<E extends BaseModel>(entityClass: any, repository?: Repository<any>): BaseService<E> {
  ...
  getService<E extends BaseModel>(entityClass: any, repository?: Repository<any>): BaseService<E> {
    return BaseService.getService<E>(entityClass, repository);
  }
  ...
  async delete<W extends any>(where: W, userId?: string): Promise<StandardDeleteResponse | StandardDeleteResponse[]> {
    const data = {
